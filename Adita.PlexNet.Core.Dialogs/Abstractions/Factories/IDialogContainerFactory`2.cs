...
namespace Adita.PlexNet.Core.Dialogs.Abstractions.Factories
{
    /// <summary>
    ...
    public interface IDialogContainerFactory<TDialog, TReturn>
        where TDialog : class, IDialog<TReturn>
    {
        ...
        /// <typeparam name="THost">The type used for the host.</typeparam>
        TContainer Create<TView, TContainer, THost>(TDialog dialog, TView view, THost? host = default)
            where TView : class
            where TContainer : class, IDialogContainer<TReturn>, new()
            where THost : class;
        #endregion Methods
    }
}
